...
(payload.*IDOC map {
			...
			documentActionCode: (if(($.EDI_DC40.MESTYP default "") == "ORDERS") "ADD" else "CHANGE_BY_REFRESH"),
			...
			orderSubType : "External Procurement",
			processType : "Supply",
			...
			supplier : {
				(primaryId : ($.*E1EDKA1 filter(($.PARVW default "") == "LF") map ($.PARTN))[0] replace /^0+/ with "") if(($.E1EDK01.BSART default "") == "NB"),
                (primaryId : ($.*E1EDKA1 filter(($.PARVW default "") == "LS") map ($.PARTN))[0] replace /^0+/ with "") if(($.E1EDK01.BSART default "") == "UB")
			},
			orderLogisticalInformation : {
				...
				($.*E1EDKA1 filter(($.PARVW default "") == "WE") map {
					shipTo : {
						primaryId : $.LIFNR,
						(if ( not isEmpty(vars.plantCodeMap[$.LIFNR])) {
							additionalPartyId : [{
								typeCode : "FOR_INTERNAL_USE_1",
								value : vars.plantCodeMap[$.LIFNR]
							}]
						} else {
						}),
			...
			deliveryTerms : {
				...
				lineItem : ( $.*E1EDP01 map ((k,v) -> using(meneeData = k.MENEE default "") {
					...
					orderLogisticalInformation : {
						...
						shipTo : {
							($.*E1EDKA1 filter(($.PARVW  default "") == "WE") map {
								primaryId : ($.LIFNR replace /^0+/ with "")
							}),
						},
					...
					lineItemDetail : (k.*E1EDP20 map (k1,v1) -> {
							...
							orderLogisticalInformation : {
								shipTo : {
									primaryId : k.WERKS,
									(if ( not isEmpty(vars.plantCodeMap[k.WERKS])) {
									additionalPartyId : [{
										typeCode : "FOR_INTERNAL_USE_1",
										value : vars.plantCodeMap[k.WERKS]
									}]
								} else {
								})
								},
