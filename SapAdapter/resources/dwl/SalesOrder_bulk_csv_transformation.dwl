...
*/
%dw 2.0
var csvCol = "itemId,demandChannel,locationId,transactionCode,startDate,quantity.value,quantity.measurementUnitCode,totalRetailAmount.value,totalRetailAmount.currencyCode" splitBy  ","
output application/csv
import lookup_utils::globalDataweaveFunctions
---
(payload pluck $$) map using (data1 = $ splitBy "_88_") {
    (csvCol[0]) : data1[1] default "",
    (csvCol[1]) : data1[7] as String default "",
    (csvCol[2]) : data1[2] default "",
    (csvCol[3]) : if (data1[3] == 'C') "111" else "UNKNOWN",
    ...
    (csvCol[7]) : payload[$].netPrice default "",
    (csvCol[8]) : data1[6] default "",
}
