...
(payload.*IDOC default [] map using (hierarchyInfo = $.E1BPE1MARART.PROD_HIER) {
			...
			senderDocumentId : $.EDI_DC40.IDOCTYP ++ '-' ++ $.EDI_DC40.DOCNUM,
			itemId : {
				// Custom added segment additionalTradeItemId for Elkjob
				(additionalTradeItemId : [
					{
						value : $.E1BPE1MARMRT.EAN_UPC,
						typeCode : "GTIN-12"
					}
				]) if ($.E1BPE1MARMRT.EAN_UPC != null and $.E1BPE1MARMRT.EAN_UPC != ""),
				primaryId : $.E1BPE1MATHEAD.MATERIAL,
				// Custom mapping in itemName for Elkjob
				itemName : $.E1BPE1MATHEAD.MATL_DESC
			},
			description : [{
				value : $.E1BPE1MAKTRT.MATL_DESC,
				languageCode : lower($.E1BPE1MAKTRT.LANGU_ISO default "en")
			}],
			// Custom added segment supplierOfTradeItem for Elkjob
			supplierOfTradeItem : [
				{
					primaryId : "NO-MAPPING-AVAILABLE",
					name : $.E1BPE1MARAEXTRT.FIELD1
				}
			],
			...
			classifications : {
				...
				// Custom added element brandName for Elkjob
				(brandName : trim(($.E1BPE1MARAEXTRT.FIELD1) [41 to 30])) if (($.E1BPE1MARAEXTRT.FIELD1 != "") and ($.E1BPE1MARAEXTRT.FIELD1 != null) and (sizeOf($.E1BPE1MARAEXTRT.FIELD1) >= 41)),
			...
			// Custom added segment financialInformation for Elkjob
			(financialInformation : {
				taxCode : $.E1BPE1MARART.TAX_CLASS as Number
			}) if (($.E1BPE1MARART.TAX_CLASS as Number default 0) >= 0),
			// Custom added segment itemDisplay for Elkjob
			itemDisplay : {
				merchandisingStyle : [{
					displayType : "TRAY",
					(width : {
						value : if ($.E1BPE1MARMRT.ALT_UNIT == "KAR") (($.E1BPE1MARMRT.WIDTH / 10) as Number) else $.E1BPE1MARMRT.WIDTH as Number
					}) if (($.E1BPE1MARMRT.WIDTH as Number default 0) > 0),
					(height : {
						value : if ($.E1BPE1MARMRT.ALT_UNIT == "KAR") (($.E1BPE1MARMRT.HEIGHT / 10) as Number) else $.E1BPE1MARMRT.HEIGHT as Number
					}) if (($.E1BPE1MARMRT.HEIGHT as Number default 0) > 0),
					(depth : {
						value : if ($.E1BPE1MARMRT.ALT_UNIT == "KAR") (($.E1BPE1MARMRT.LENGTH / 10) as Number) else $.E1BPE1MARMRT.LENGTH as Number
					}) if (($.E1BPE1MARMRT.LENGTH as Number default 0) > 0),
					(totalQuantityOfUnits : if ($.E1BPE1MARMRT.ALT_UNIT == "KAR") (($.E1BPE1MARMRT.NUMERATOR as Number) / ($.E1BPE1MARMRT.DENOMINATR as Number) as Number) else 1) if (($.E1BPE1MARMRT.NUMERATOR as Number default 0) > 0)
				}]
			},
			operationalRules : {
				trackingInformation : {
